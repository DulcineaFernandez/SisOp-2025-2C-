...
trap_dispatch(struct Trapframe *tf)
{
	...
	case IRQ_TIMER:
		...
		if (curenv && curenv->env_status == ENV_RUNNING) {
			curenv->runtime++;
			curenv->vruntime +=
			        curenv->priority;  // penalizamos el vruntime,segun
			                           // la prioridad (menor prioridad
			                           // numerica = crece mas lento el vruntime)
		}
		extern void sched_boost(void);
		sched_boost();  // porque no queremos que algunos procesos "mueran" de starvation
#endif
