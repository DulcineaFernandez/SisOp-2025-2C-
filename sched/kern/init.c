...
#ifndef TEST
	// TEST DEL SCHEDULER CON PRIORIDADES

	cprintf("\n--- TEST: Scheduler con Prioridades ---\n");

	// Creamos 3 procesos de usuario iguales (test_prio)
	ENV_CREATE(user_test_prio, ENV_TYPE_USER);
	ENV_CREATE(user_test_prio, ENV_TYPE_USER);
	ENV_CREATE(user_test_prio, ENV_TYPE_USER);

	cprintf("Se crearon 3 procesos user_test_prio\n");
	cprintf("Cada uno mostrara su prioridad, iteraciones y cambios\n\n");
#endif
#endif
