...
sys_get_priority(envid_t envid)
{
	...
	if (envid == 0) {
		e = curenv;
	} else if (envid2env(envid, &e, true) < 0) {
		return -E_BAD_ENV;
	}
	return e->priority;
}
...
static int
sys_set_priority(envid_t envid, int nuevaprio)
{
	struct Env *e;
	if (envid == 0) {
		e = curenv;
	...
	if (nuevaprio < HIGHEST_PRIORITY || nuevaprio > LOWEST_PRIORITY) {
		return -E_INVAL;
	}
	if (nuevaprio < e->priority) {
		return -E_INVAL;
	}
	e->priority = nuevaprio;
	return 0;
}
#endif
