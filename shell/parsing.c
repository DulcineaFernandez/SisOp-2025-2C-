...
expand_environ_var(char *arg)
{
	...
	if (arg[0] == '$') {
		if (strcmp(arg + 1, "?") == 0) {
			char value_status[20];
			sprintf(value_status, "%i", status);
			if (strlen(value_status) > strlen(arg + 1)) {
				arg = realloc(arg, strlen(value_status) + 1);
			}
			strcpy(arg, value_status);
			return arg;
		}

		char *value_env = getenv(arg + 1);
		if (value_env == NULL || value_env[0] == '\0') {
			return NULL;
		}

		if (strlen(value_env) > strlen(arg + 1)) {
			arg = realloc(arg, strlen(value_env) + 1);
		}
		strcpy(arg, value_env);
	}
